...
function checkObject(object) { object.checked = true; }
...
function arraySet(array, index, number) {
    if(index in array) {
        array[index] = number;
    }
}
...
// function addToAll(array, adder) { return array.reduce((i) => i + adder); }
...
function addAllOpt(array) {
    if(Array.isArray(array)) {
        return array.reduce((head, tail) => head + tail, 0);
    } else { return 0;}
}
...
function multiples(n, m) {
    return Array.from({length: n}, (index, numbers)  => 1*m, );
}